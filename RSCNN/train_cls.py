...
def validate(test_dataloader, model, criterion, args, iter): 
    ...
    with torch.no_grad():
        ...
        # Convert the list of tensors to a single tensor, move to CPU, then take the mean
        val_loss_mean = torch.stack(losses).cpu().mean().item()
